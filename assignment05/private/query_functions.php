<?php
  ...
  function validate_user($user, $errors=array()) {
    ...
    if (is_blank($user['password'])) {
      ...
      $errors[] = "Password must be more than 12 and less than 255 characters.";
    } elseif (!has_valid_password_format($user['password'])) {
      $errors[] = "Password needs to contain at least one of each: uppercase letter, lowercase letter, number, symbol.";
    }
    
    if (!has_matching_passwords($user['password'], $user['password_confirmation'])) {
  ...
  // Add a new user to the failed_logins table
  ...
  // Edit a user record in the failed_logins table
  ...
  // Resets a username's count in failed_logins table
