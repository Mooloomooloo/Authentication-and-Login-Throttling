<?php
  ...
  // has_valid_password_format('Hello@World!')
  function has_valid_password_format($value) {
    $uppercase = preg_match('@[A-Z]@', $value); // checks if there is an uppercase
    $lowercase = preg_match('@[a-z]@', $value); // checks if there is a lowercase
    $number = preg_match('@[0-9]@', $value); // checks if there is a number
    $special_char = preg_match('@[^A-Za-z0-9\s]@', $value); // checks if there is a special character
    return ($uppercase && $lowercase && $number && $special_char);
  }
  
  // Checks if password and password confirmation match
  function has_matching_passwords($value1, $value2) {
